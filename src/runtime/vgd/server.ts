  ...
  const server = net.createServer((socket) => {
    ...
    socket.on('data', (chunk: string) => {
      ...
      while ((nl = buffer.indexOf('\n')) >= 0) {
        ...
        if (!line) continue;
        const response = handleLine(line, { registry, now, pid, startedAt, socketPath });
...
function handleLine(
  line: string,
  ctx: { registry: WorkspaceRegistry; now: () => Date; pid: number; startedAt: number; socketPath: string },
): VgdResponse {
