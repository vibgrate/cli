 ...
 *
 * Every scanned package is placed in one of three tiers — `'micro'`, `'small'`
 * or `'standard'` — based on three measured signals: source-file count, source
 * byte size (lockfiles, generated code and vendored directories excluded) and
 ...
 *
 * The ~3× jump between adjacent bands (rather than a single 10× cliff) keeps
 * boundary disputes small and predictable.
 */

// ── Micro tier (smallest, billed at 1/10) ──

/** A micro project has fewer than this many source files. */
export const MICRO_MAX_FILES = 10;

/** A micro project's source files total fewer than this many bytes (1 MB). */
export const MICRO_MAX_SIZE_BYTES = 1_048_576;
...
/**
 * Classify a project as `'micro'`, `'small'` or `'standard'`.
 *
 ...
 * 1/{@link SMALL_BILLING_RATIO} and each micro project as
 * 1/{@link MICRO_BILLING_RATIO}. `billableProjectsRaw` is the exact fractional
 ...
 * micro projects, still rolls up fully into drift scores and the portfolio
 * view; only the billing weight is reduced.
 */
export function summarizeBilling(projects: ProjectScan[]): BillingSummary {
  ...
  const billableProjectsRaw =
    standardCount + smallCount / SMALL_BILLING_RATIO + microCount / MICRO_BILLING_RATIO;

  return {
    microCount,
    smallCount,
    standardCount,
    totalScanned: microCount + smallCount + standardCount,
