...
export class ScanProgress {
  ...
  private trace: ProgressTrace | null;

  constructor(rootDir: string, version = 'unknown') {
  ...
  setSteps(steps: Array<{ id: string; label: string; weight?: number }>): void {
    ...
    if (this.isTTY) {
      ...
      const header = [
        '',
        `  ${ROBOT[0]}  ${getBrand(this.version)[0]}`,
        `  ${ROBOT[1]}  ${getBrand(this.version)[1]}`,
        `  ${ROBOT[2]}`,
  ...
  private traceStats(force = false): void {
    if (!this.trace) return;
    const snapshot = {
      stats: {
        ...this.stats,
        findings: { ...this.stats.findings },
        treeSummary: this.stats.treeSummary ? { ...this.stats.treeSummary } : undefined,
      },
    };
