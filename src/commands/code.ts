      ...
      if (!opts.model && !opts.modelPath && (modeFlag || (!provider && !model))) {
        try {
          ...
          if (resolved.fit.label === 'will_not_fit' && !opts.mock) {
            throw new CliError(
              `Code Mode ${resolved.mode} will not fit on this machine (${resolved.fit.reasons[0] ?? 'insufficient memory'}). Try \`vg models ensure spark --yes\` or free RAM.`,
              ExitCode.ERROR,
            );
          }
          provider = provider ?? providerForBackend(resolved.underlying.backend);
          model = model ?? resolved.underlying.weightsRef;
